...
def minutosTotales(request):
    ...
    try:
        ...
        print ("jala3")
        cursor.execute("SELECT DATEDIFF(minute, ended, started) AS TiempoSesion,SUM(num_compounds_made) AS SumaCompuestos FROM \"WEB_day\" INNER JOIN \"WEB_try\" ON \"WEB_day\".try_id_id=\"WEB_try\".id INNER JOIN \"WEB_sesion\" ON session_id= id GROUP BY session_id ORDER BY TiempoSesion ASC")
