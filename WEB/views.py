...
def minutosTotales(request):
    ...
    try:
        ...
        print ("jala3")
        cursor.execute("SELECT DATE_PART('hour', ended::timestamp, started::timestamp) AS TiempoSesion,SUM(num_compounds_made) AS SumaCompuestos FROM \"WEB_day\" INNER JOIN \"WEB_try\" ON \"WEB_day\".try_id_id=\"WEB_try\".id INNER JOIN \"WEB_sesion\" ON \"WEB_try\".session_id_id= \"WEB_sesion\".id GROUP BY session_id ORDER BY TiempoSesion ASC")
